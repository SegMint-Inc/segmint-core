...
import { ISegMintFactory } from "./interfaces/ISegMintFactory.sol";
...
/**
 * @title SegMintFactory
 * @notice See documentation for {ISegMintFactory}.
 */

contract SegMintFactory is ISegMintFactory, OwnableRoles, UpgradeHandler, Initializable, EIP712 {
    ...
    function createVault(bytes calldata signature) external override {
        ...
        emit ISegMintFactory.VaultCreated({ user: msg.sender, vault: newVault });
    }
    ...
    function createSafe(bytes calldata signature, address[] calldata signers, uint256 quorum) external override {
        ...
        emit ISegMintFactory.SafeCreated({ user: msg.sender, safe: newSafe });
    }
